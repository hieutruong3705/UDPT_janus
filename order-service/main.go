...
func orders(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(map[string]interface{}{
		...
		"orders":  []string{"Order_1", "Order_2", "Order_3"},
	})
}
...
func crash(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(map[string]string{"message": "order-service is crashing for demo"})
...
func stress(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]interface{}{
		"message":      "order-service accepted stress memory block",
...
func main() {
	...
	println("Order Service is running on port :" + myPort)
